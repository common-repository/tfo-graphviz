...
// $Id: tfo-graphviz-admin.php 2110959 2019-06-23 18:03:09Z chrisy $
...
<?php
	...
	function update($new) {
		...
		$this->options = compact('css', 'graphviz_path', 'method', 'maxage', 'emitjs');
		update_option('tfo-graphviz', $this->options);
	...
	function test_image() {
		...
		$graphviz_object = $this->graphviz('digraph test { graph [bgcolor="transparent"] a1 -> a2 -> a3 -> a1; }', array(
				'id' => 'test',
				'lang' => 'dot',
				'simple' => 'false',
				'output' => 'png',
		...
		} else if($graphviz_object->emits_inline()) {
			echo $graphviz_object->inline();
		}
	...
	function admin_page() {
		...
		foreach ( $this->methods as $class => $method ) {
			...
			try {
				...
				$unavailable_methods[$class] = "Exception during method load: " + $e->getMessage();
				continue;
			}
			if ($included === FALSE) { // module didn't load, or indicated it was not usable
				$unavailable_methods[$class] = "Module refused to load;" .
				(empty($tfo_include_error) ? " possibly some dependencies are missing." : " $tfo_include_error.");
				continue;
			}
			...
			try {
				...
				$unavailable_methods[$class] = "Exception when testing the method: " + $e->getMessage();
				continue;
			}
...
?>
	...
	<tbody>
		...
		<tr>
			...
			'<code>[graphviz]digraph test { graph [bgcolor="transparent"] a1 -> a2 -> a3 -> a1; }[/graphviz]</code>'
		); ?></p>
		...
		<tr<?php if ( in_array( 'method', $errors ) ) echo ' class="form-invalid"'; ?>>
			...
			<td class="syntax">
				<p>Only available methods will be selectable. If you have Graphviz installed locally and no options are selectable, make sure the
...
<?php
		foreach ($available_methods as $class => $method) {
			?><li><label for="tfo-graphviz-method-<?php echo $method;?>"><input type="radio" name="tfo-graphviz[method]" id="tfo-graphviz-method-<?php echo $method;?>" value='<?php echo $class;?>'<?php checked($class, $values['method']); ?> /> <code><?php echo esc_html($method) ?></code>: <?php _e($this->method_label[$method], 'tfo-graphviz'); ?></label></li><?php
		}
		?></ul>
		<p>Unavailable methods:</p>
		<ul><?php
		// now show the unavailable methods
		foreach ($unavailable_methods as $class => $message) {
			?><li><label for="tfo-graphviz-method-<?php echo $class; ?>"><input type="radio" disabled> <code><?php echo esc_html($this->methods[$class]); ?></code>: <?php _e($message, 'tfo-graphviz'); ?></label></li><?php
		}
		?></ul>
			</td>
		</tr>
		...
		<tr class="tfo-graphviz-emitjs<?php if ( in_array( 'graphviz_emitjs', $errors ) ) echo ' form-invalid'; ?>">
			...
			<td><input type="checkbox" name="tfo-graphviz[emitjs]" value="1"<?php echo $values['emitjs'] ? ' checked="checked"' : ''; ?> id="tfo-graphviz-emitjs">
				Select this to emit JavaScript when necessary; deselect to never emit JS (this may break certain output methods and options.)
			</input></td>
		</tr>
...
<?php
	...
	function activation_hook() {
		...
		$this->options = compact('method', 'css', 'graphviz_path', 'maxage', 'activated', 'emitjs');
		update_option( 'tfo-graphviz', $this->options );
	}
